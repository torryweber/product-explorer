...
function showScreen(name) {
  ...
  // Force next screen to top immediately
...
function renderBrands() {
  ...
  DATA.brands.forEach((brand, i) => {
    ...
    item.innerHTML = `
      <div class="icon" style="background: ${brand.color}">${brand.icon}</div>
      <span class="icon-label">${brand.name}</span>
    `;
...
function renderProducts(category) {
  ...
  category.products.forEach((prod, i) => {
...
function renderDetail(product) {
  ...
  productDetail.innerHTML = `
    <div class="detail-hero">${product.emoji}</div>
    <div class="detail-info">
      <h3>${product.name}</h3>
      <div class="detail-price">${product.price}</div>
      <p class="detail-desc">${product.desc}</p>
      <div class="detail-meta">
        <span class="meta-chip">${currentBrand.name}</span>
        <span class="meta-chip">${currentCategory.name}</span>
        ${product.tag ? `<span class="meta-chip">${product.tag}</span>` : ""}
      </div>
      <button class="cta-btn" id="add-to-cart">Add to Cart</button>
    </div>
  `;
